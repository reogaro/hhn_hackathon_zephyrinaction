...
/* How long the restart message stays on screen. */
...
static uint8_t cbuf[LV_CANVAS_BUF_SIZE(GAME_W, GAME_H, 16, LV_DRAW_BUF_STRIDE_ALIGN)];
...
int main(void)
{
    ...
    /* The canvas only covers the game area: a full-screen RGB565 buffer would
     * not fit in the 2 MB of cached RAM left below the framebuffers. */
    lv_obj_set_style_bg_color(lv_scr_act(), lv_color_white(), 0);
    canvas = lv_canvas_create(lv_scr_act());
    lv_canvas_set_buffer(canvas, cbuf, GAME_W, GAME_H, LV_COLOR_FORMAT_RGB565);
    lv_obj_set_pos(canvas, GAME_OFFSET_X, GAME_OFFSET_Y);
    draw_static_maze();
    ...
    /* Banner above the maze, shown briefly after every restart. */
    lv_obj_t *banner = lv_label_create(lv_scr_act());
    lv_obj_set_style_text_font(banner, &lv_font_montserrat_32, 0);
    lv_obj_set_style_text_color(banner, lv_color_make(30, 40, 80), 0);
    ...
    while (1) {
        ...
        for (int i = 0; i < N_PITS; i++) {
            ...
            if (sqrtf(dx * dx + dy * dy) < pits[i].r) {
                ...
                lv_label_set_text(banner, "You fell in a hole! Restarting...");
                lv_obj_align(banner, LV_ALIGN_TOP_MID, 0, 10);
        ...
        }
        {
            ...
            if (sqrtf(dx * dx + dy * dy) < goal.r) {
                ...
                x = START_X; y = START_Y; vx = 0; vy = 0;
                lv_label_set_text(banner, "You made it out! Restarting...");
                lv_obj_align(banner, LV_ALIGN_TOP_MID, 0, 10);
                lv_obj_remove_flag(banner, LV_OBJ_FLAG_HIDDEN);
                banner_until = now + BANNER_MS;
            }
