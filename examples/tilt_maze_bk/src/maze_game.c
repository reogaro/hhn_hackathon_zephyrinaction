...
/* Ball this close to the start counts as "ready to start" for the LED ring */
#define START_ZONE_RADIUS  25.0f
...
static void game_step(uint32_t now)
{
	...
	float sx = ball_x - MAZE_START_X;
	float sy = ball_y - MAZE_START_Y;

	led_ring_set_state(sqrtf(sx * sx + sy * sy) < START_ZONE_RADIUS ?
			   LED_RING_READY : LED_RING_PLAYING);
	...
	for (size_t i = 0; i < n_pits; i++) {
		...
		if (sqrtf(dx * dx + dy * dy) < pits[i].r) {
			...
			led_ring_flash(LED_RING_FLASH_LOSS);
