...
| 2    | led_ring | WS2812 ring (SPI1, polled): shows the game state (shares core 2) |
...
A 24-LED WS2812 ring shows the game state. Wire DIN to the mikroBUS MOSI pin, 5V and GND to the ring, with a common ground. No pull-up is needed. The LED count is `chain-length` in `app.overlay`; brightness is `BRIGHTNESS` in `src/led_ring.c`. What it shows:

| Situation | Ring |
| --------- | ---- |
| Game starting | all LEDs blink blue 3 times |
| Ball at the start, waiting | yellow |
| Ball rolling | green |
| Ball fell in a pit | red blinks 3 times, then back to yellow |
| Button long press (reset) | yellow blinks 2 times, then back to yellow |
 Without a ring connected the game runs normally.
